...
def vk_stats():
    ...
    if not user.vk_api_id or not user.vk_group_id:
        flash('Пожалуйста, укажите VK API ключ и ID группы в настройках.', 'warning')
        return redirect(url_for('smm.settings'))

    stats = {}
    try:
        vk_stats_manager = VKStats(user.vk_api_id, user.vk_group_id)

        followers_count = vk_stats_manager.get_followers()

        # Определяем период: последние 30 дней
        end_date = datetime.now()
        start_date = end_date - timedelta(days=7)

        # Передаем объекты datetime напрямую в обновленный метод
        activity_data = vk_stats_manager.get_activity_stats(start_date, end_date)

        stats = {
            "Подписчики (всего)": followers_count,
            f"Лайки (за последние 7 дней)": activity_data.get('likes', 'N/A'),
            f"Комментарии (за последние 7 дней)": activity_data.get('comments', 'N/A'),
            f"Репосты (за последние 7 дней)": activity_data.get('shares', 'N/A')
        }

    except Exception as e:
        flash(f'Не удалось получить статистику из VK: {e}', 'danger')
        return render_template('vk_stats.html', stats=None)
